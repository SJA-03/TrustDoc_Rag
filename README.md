...
평가는 5개 질문으로 진행했으며, 하나의 질문에 여러 페이지가 유효한 근거가 될 수 있어 복수 정답 페이지를 지원했습니다.

사용한 지표:

| Metric | Meaning |
|---|---|
| Hit@1 | 정답 근거가 검색 결과 1등에 포함되었는가 |
| Hit@3 | 정답 근거가 top 3 안에 포함되었는가 |
| Hit@5 | 정답 근거가 top 5 안에 포함되었는가 |
| MRR | 정답 근거가 몇 번째 순위에 나왔는지를 반영한 평균 점수 |

Evaluation result:

| Collection | Hit@1 | Hit@3 | Hit@5 | MRR |
|---|---:|---:|---:|---:|
| paragraph | 0.8000 | 1.0000 | 1.0000 | 0.8667 |
| fixed-size | 0.8000 | 0.8000 | 1.0000 | 0.8400 |

현재 평가에서는 paragraph/page 기반 chunking이 fixed-size chunking보다 Hit@3과 MRR에서 더 안정적인 결과를 보였습니다.
